...
class ApolloClient:
    ...
    async def enrich_person(self, linkedin_url: str = "", email: str = "") -> dict[str, Any]:
        if not self.api_key:
            return {"id": None, "error": "Apollo not configured"}
        try:
            ...
            if email:
                body["email"] = email
            resp = await (await self._session()).post(
    ...
    async def search_people(self, query: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            resp.raise_for_status()
            return resp.json().get("people", []) or resp.json().get("results", [])
        except Exception:
            return []
    ...
    async def search_organizations(self, query: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            resp.raise_for_status()
            return resp.json().get("organizations", []) or resp.json().get("results", [])
        except Exception:
            return []
    ...
    async def verify_email(self, email: str) -> dict[str, Any]:
        ...
        try:
            resp = await (await self._session()).post(
                f"{self.base_url}/people/verify_email",
                headers={"Content-Type": "application/json", "X-API-KEY": self.api_key},
