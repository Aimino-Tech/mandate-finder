...
async def ingest_postings(postings: list[JobPosting]) -> dict[str, Any]:
    _POSTINGS.extend(postings)
    return {"ingested": len(postings), "total": len(_POSTINGS)}


@router.post("/signals")
async def ingest_signals(signals: list[EarlySignal]) -> dict[str, Any]:
...
async def export_trends_csv(postings: PostingsDep, signals: SignalsDep, days: int = Query(90, ge=1, le=365), fmt: str = Query("csv")) -> Response:
...
async def get_top_roles(postings: PostingsDep, days: int = Query(90, ge=1, le=365), limit: int = Query(10, ge=1, le=50)) -> dict[str, Any]:
